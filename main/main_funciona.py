...
from logic.generador import generar_valida
from utils.helpers import limpiar_frame
...
# FILTROS
# -----------------------------
def obtener_configuracion_filtros():
    return {
        "bajos_altos": combo_bajos_altos.get(),
        "pares_impares": combo_pares_impares.get(),
        "suma_min": suma_min.get(),
        "suma_max": suma_max.get(),
        "max_consec": max_consec.get(),
        "decenas": decenas_input.get()
    }
...
# -----------------------------
def generar_1():
    config = obtener_configuracion_filtros()
    c = generar_valida(config)
